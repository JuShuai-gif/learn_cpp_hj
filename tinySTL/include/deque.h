...
struct deque_buf_size {
    // 模板结构体 deque_buf_size，它包含一个静态成员变量 value，用于计算某种类型 T 的缓冲区大小
...
private:
    ...
    map_pointer map_;    // 指向一块 map, map 中的每一个元素都是一个指针，指向一个缓冲区
...
public:
    ...
    /*
    noexcept 关键字用于指定函数不会抛出异常,好处：1.性能优化。2.明确代码意图。3.提高异常安全性
    使用场所：
    1. 简单操作函数。
    2. 容器和指针操作函数。
    3. 标准库中标记为 noexcept 的函数。
    4. 构造函数和析构函数。
    5. 移动构造函数和移动赋值运算符
    */
    ...
    // std::enable_if 是 C++ 标准库中的一个模板，它主要用于在编译时进行条件判断，从而启用或禁用特定的函数或模板特化
    template <class IIter, typename std::enable_if<
                               mystl::is_input_iterator<IIter>::value, int>::type = 0>
...
void deque<T>::shrink_to_fit() noexcept {
    // map_是一个大的缓冲区，而deque的数据就在这个缓冲区中，不一定在开头，也不一定在结尾，所以要遍历两个部分
    // 至少会留下头部缓冲区
