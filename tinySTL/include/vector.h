...
public:
    ...
    // 指向 valu_type 的指针类型，用于遍历和访问容器中的元素
...
void vector<T>::try_init() noexcept {
    try {
        ...
        // begin_ 和 end_ 被设置为相同的地址，这通常表示在初始化阶段，
        // vector 对象内部并没有实际存储任何元素，因此 begin_ 和 end_ 都指向同一个位置，即分配的起始地址
        // 随着向 vector 中添加元素，end_ 会随之改变，指向最后一个有效元素之后的位置
        end_ = begin_;
        // 初次分配的空间是16
        cap_ = begin_ + 16;
    } catch (...) {
...
void vector<T>::
    ...
    // 将迭代器Iter begin到end之间的元素复制到begin_开头的位置
...
void vector<T>::
    ...
    /*
    为什么需要在销毁元素后再释放内存呢？这是因为在很多情况下，对象的析构函数可能会有资源释放的工作，
    例如释放动态分配的内存或者关闭文件等。
    因此，在释放内存之前，必须确保所有对象的析构函数都已经被正确调用，以避免资源泄漏或者未定义行为。
    */
    // 其实就是调用元素的 析构函数
...
vector<T>::
    ...
    // 要么增加 当前容量的一半（old_size / 2）加上当前容量
    // 要么增加 old_size + add_size
